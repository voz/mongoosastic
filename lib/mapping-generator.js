...
function getMapping(cleanTree, prefix) {
  ...
  for (var field in cleanTree) {
    ...
    for (var prop in value) {
      ...
      if (prop.indexOf('es_') === 0 && prop !== 'es_indexed' && prop !== 'es_parent') {
        mapping[field][prop.replace(/^es_/, '')] = value[prop];
      }

      // TODO: make sure that there is only one parent specified, err
      if (prop === 'es_parent') { // treat this field as a parent
        _parent.type = value['ref'].toLowerCase();
      }
    }
  }
